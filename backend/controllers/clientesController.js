...
export const registrarCliente = async (req, res) => {
  try {
    ...
    if (!cliente.titular1 || cliente.titular1.trim() === "") {
      return res.status(400).json({ error: "El titular 1 es obligatorio." });
    }

    // Validación de etapa si es Los Eucaliptos
    if (cliente.barrio === "Los Eucaliptos") {
      const etapasValidas = ["1", "2", "3", "4"];
      if (!etapasValidas.includes(cliente.etapa)) {
        return res.status(400).json({ error: "Etapa inválida para Los Eucaliptos." });
      }
    }

    // Evitar duplicados (lote + manzana + etapa + barrio)
    const { data: existente } = await supabase
      .from("clientes")
      .select("*")
      .eq("barrio", cliente.barrio)
      .eq("numero_lote", cliente.numero_lote)
      .eq("manzana", cliente.manzana)
      .eq("etapa", cliente.etapa);

    if (existente && existente.length > 0) {
      return res.status(400).json({ error: "Ya existe un cliente con este lote, manzana y etapa." });
    }
